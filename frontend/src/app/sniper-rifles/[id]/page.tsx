...
export default function SniperRifleDetailPage() {
  ...
  const [rifle, setRifle] = useState<SniperRifle | null>(null);
  ...
  const fetchRifleDetails = async () => {
    try {
      ...
      if (result.success) {
        ...
        const current = rifles.find((r: SniperRifle) =>
        ...
        if (current) {
          ...
          setError(`Sniper Rifle with ID "${rifleId}" not found`);
        }
      } else {
        setError('Failed to fetch sniper rifle data');
      }
    } catch (err) {
      setError('Error fetching sniper rifle details: ' + (err as Error).message);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center" style={{
        ...
        <div className="text-center">
          ...
          <p className="text-white mt-4">Loading sniper rifle details...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="p-8 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-3 mb-6">
                  <div><p className="text-sm text-gray-400">Category</p><p className="text-lg text-white font-semibold">{rifle.category}</p></div>
                  <div><p className="text-sm text-gray-400">Origin</p><p className="text-lg text-white font-semibold">{rifle.origin}</p></div>
                </div>
