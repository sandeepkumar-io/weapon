        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-8">
            <p className="text-gray-400">
              ...
              <span className="text-blue-400 font-semibold">{data.length}</span> rifles
            </p>
          </div>
        </div>
      </div>
      ...
      {loading && (
        <div className="flex items-center justify-center min-h-96">
          <div className="text-center">
            ...
            <p className="text-white mt-4">Loading rifles...</p>
          </div>
        </div>
      )}
      ...
      {!loading && (
        <div className="max-w-7xl mx-auto px-4 md:px-8 pb-20">
          {filteredData.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredData.map((item) => (
                <DataCard
                  ...
                  detailPath={`/rifles/${encodeURIComponent(item.id)}`}
                />
              ))}
            </div>
          ) : (
            <div className="flex items-center justify-center min-h-96">
              <div className="text-center">
                <p className="text-gray-400 text-lg">No rifles found</p>
              </div>
            </div>
          )}
        </div>
